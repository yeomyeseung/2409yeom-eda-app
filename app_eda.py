...
class Home:
    def __init__(self, login_page, register_page, findpw_page):
        ...
        st.markdown("""
                ...
                **전국 시·도별 인구 추세 데이터셋**
                - 출처: 통계청 또는 교육부 지역 인구 통계 (예시)
                - 설명: 2008년부터 2022년까지 전국 및 각 시·도의 연도별 인구, 출생자 수, 사망자 수가 기록된 데이터
                - 주요 변수:
                  - `연도`: 기준 연도  
                  - `지역`: 전국, 서울, 부산 등 지역명  
                  - `인구`: 해당 연도의 총인구 수  
...
class EDA:
    def __init__(self):
        ...
        # 9. 인구 추세 분석
        with tabs[8]:
            st.header("👥 인구 추세 분석 (population_trends.csv)")
            pop_file = st.file_uploader("인구 데이터 업로드 (population_trends.csv)", type="csv", key="pop")
            ...
            pop_tabs = st.tabs([
                "1) 기초 통계", "2) 연도별 추이", "3) 지역별 분석", "4) 변화량 분석", "5) 시각화"
            ])

            # 1) 기초 통계
            with pop_tabs[0]:
                st.subheader("결측치 및 중복 체크")
                st.write("결측치 개수:")
                st.dataframe(df_pop.isnull().sum())

                st.write(f"중복 행 개수: {df_pop.duplicated().sum()}")
            ...
            # 2) 연도별 전체 인구 추이
            with pop_tabs[1]:
                st.subheader("연도별 전체 인구 추이 (전국 기준)")
                df_national = df_pop[df_pop['지역'] == '전국']
                fig, ax = plt.subplots()
                sns.lineplot(x='연도', y='인구', data=df_national, marker='o', ax=ax)
                ax.set_ylabel("전국 인구수")
                st.pyplot(fig)

            # 3) 지역별 인구 변화량 순위
            with pop_tabs[2]:
                st.subheader("지역별 인구 변화량 (증감 총합 기준)")
                df_latest = df_pop.groupby(['지역', '연도'])['인구'].sum().reset_index()
                pivot = df_latest.pivot(index='연도', columns='지역', values='인구')
                delta = pivot.diff().sum().sort_values(ascending=False)

                st.dataframe(delta.rename("누적 변화량").to_frame())

            # 4) 증감률 상위 지역 및 연도
            with pop_tabs[3]:
                st.subheader("연도별 지역 인구 증감률 상위")
                df_sorted = df_latest.sort_values(['지역', '연도'])
                df_sorted['변화율'] = df_sorted.groupby('지역')['인구'].pct_change() * 100

                top_changes = df_sorted.sort_values('변화율', ascending=False).dropna().head(10)
                st.dataframe(top_changes[['연도', '지역', '인구', '변화율']])

            # 5) 누적 영역 그래프
            with pop_tabs[4]:
                st.subheader("누적 영역 그래프 (지역별 인구)")
                pivot_area = df_latest.pivot(index='연도', columns='지역', values='인구')
                fig, ax = plt.subplots(figsize=(10, 6))
                pivot_area.plot.area(ax=ax, legend=True, alpha=0.6)
                ax.set_ylabel("인구 수")
                st.pyplot(fig)
